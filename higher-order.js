...
let evenNumbers = mixedNumbers.filter((x, i, arr)=>x%2===0); // = mixedNumbers.filter(/* Provide Your Callback Here */)
...
let postTaxPrices = prices.map((x, i, arr)=>{
    x+= (x * 1.07);
...
let totalPopulation = populations.reduce((p, x, i, arr)=>p+x); //  = populations.reduce(/* Provide Your Callback Here */)
...
let myStrongest = monstersInYourPocket.filter((x, i, arr)=> arr[i].CP > 200);// Your code here
...
let ordersTotal = orders.map((x, i, arr)=>x+=(arr[i].price+(arr[i].price * arr[i].tax))); // Your code here
